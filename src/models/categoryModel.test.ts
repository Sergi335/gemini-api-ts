...
describe('categoryModel.getAllCategories', () => {
  ...
  it('devuelve categorías del usuario', async () => {
    ...
    const result = await categoryModel.getAllCategories({ user: 'user1' })
  ...
  it('devuelve array vacío si no hay categorías', async () => {
    ...
    const result = await categoryModel.getAllCategories({ user: 'user2' })
...
describe('categoryModel.getCategoryByParent', () => {
  ...
  it('devuelve categorías si existen para el parent', async () => {
    ...
    const result = await categoryModel.getCategoryByParent({ user: 'user1', parentId: 'parent1' })
    expect(result).toEqual(mockData)
  })
  ...
  it('devuelve error si no existen categorías para el parent', async () => {
    ...
    const result = await categoryModel.getCategoryByParent({ user: 'user1', parentId: 'parentX' })
...
describe('categoryModel.getCategoryCount', () => {
  ...
  it('devuelve el número de categorías si hay', async () => {
    ...
    const result = await categoryModel.getCategoryCount({ user: 'user1' })
  ...
  it('devuelve error si no hay categorías', async () => {
    ...
    const result = await categoryModel.getCategoryCount({ user: 'user2' })
...
describe('categoryModel.createCategory', () => {
  ...
  it('crea una categoría correctamente', async () => {
    ...
    const result = await categoryModel.createCategory({ user: 'user1', cleanData: mockCategory })
    expect(result).toEqual([mockCreated])
  })

  it('lanza error si el nombre está vacío', async () => {
    await expect(categoryModel.createCategory({ user: 'user1', cleanData: { name: '' } })).rejects.toThrow('Category name is required to generate a slug')
  })
})
...
describe('categoryModel.updateCategory', () => {
  ...
  it('actualiza el nombre y el slug de una columna', async () => {
    ...
    const result = await categoryModel.updateCategory({ user: 'user1', id: 'col1', cleanData: { name: 'Updated Name' } })
    expect(categoryModel.generateUniqueSlug).toHaveBeenCalledWith({ user: 'user1', name: 'Updated Name' })
  ...
  it('mueve una columna a otro escritorio y actualiza el orden', async () => {
    ...
    // @ts-expect-error
    link.updateMany.mockReturnValue({ session: vi.fn().mockResolvedValue({ acknowledged: true, matchedCount: 1, modifiedCount: 1, upsertedCount: 0, upsertedId: new Types.ObjectId() }) })
    const result = await categoryModel.updateCategory({ user: 'user1', id: 'col1', cleanData: { parentId: 'desk2' } })
  ...
  it('devuelve un error si la columna a actualizar no se encuentra', async () => {
    ...
    // @ts-expect-error
    link.updateMany.mockReturnValue({ session: vi.fn().mockResolvedValue({ acknowledged: true, matchedCount: 1, modifiedCount: 1, upsertedCount: 0, upsertedId: new Types.ObjectId() }) })
    const result = await categoryModel.updateCategory({ user: 'user1', id: 'nonexistent', cleanData: { name: 'any' } })
  ...
  it('maneja errores durante la transacción y hace abort', async () => {
    ...
    // @ts-expect-error
    link.updateMany.mockReturnValue({ session: vi.fn().mockResolvedValue({ acknowledged: true, matchedCount: 1, modifiedCount: 1, upsertedCount: 0, upsertedId: new Types.ObjectId() }) })
    const result = await categoryModel.updateCategory({ user: 'user1', id: 'col1', cleanData: { name: 'any' } })
...
describe('categoryModel.deleteCategory', () => {
  ...
  it('elimina una columna y sus links asociados', async () => {
    ...
    const result = await categoryModel.deleteCategory({ user: 'user1', id: 'col1' })
